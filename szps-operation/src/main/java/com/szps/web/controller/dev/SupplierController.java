package com.szps.web.controller.dev;

import java.util.List;

import org.apache.shiro.authz.annotation.RequiresPermissions;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.ModelMap;
import org.springframework.validation.annotation.Validated;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseBody;

import com.szps.common.annotation.Log;
import com.szps.common.constant.UserConstants;
import com.szps.common.core.controller.BaseController;
import com.szps.common.core.domain.AjaxResult;
import com.szps.common.core.domain.Ztree;
import com.szps.common.core.page.TableDataInfo;
import com.szps.common.enums.BusinessType;
import com.szps.framework.util.ShiroUtils;
import com.szps.system.domain.SysDept;
import com.szps.system.domain.SysUser;
import com.szps.web.domain.dev.Supplier;
import com.szps.web.service.dev.ISupplierService;

@Controller
@RequestMapping("/op/dev/supplier")
public class SupplierController extends BaseController {
	@Autowired
	private ISupplierService service;

	private String prefix = "/dev/supplier";

	@RequiresPermissions("dev:supplier:view")
	@GetMapping()
	public String view() {
		return prefix + "/view";
	}

	/**
	 */
	@RequiresPermissions("dev:supplier:view")
	@PostMapping("/list")
	@ResponseBody
	public TableDataInfo list(Supplier obj) {
		startPage();
		List<Supplier> list = service.selectList(obj);
		return getDataTable(list);
	}

	/**
	 * 新增参数配置
	 */
	@GetMapping("/add")
	public String add(ModelMap mmap) {
		SysUser user = ShiroUtils.getSysUser();
		SysDept sysDept = user.getDept();
		mmap.put("sysDept", sysDept);
		return prefix + "/add";
	}

	/**
	 * 新增保存日报
	 */
	@RequiresPermissions("dev:supplier:add")
	@Log(title = "添加供应商信息", businessType = BusinessType.INSERT)
	@PostMapping("/add")
	@ResponseBody
	public AjaxResult addSave(Supplier obj) {
		if (UserConstants.UNIQUE.equals(service.checkUnique(obj)))
        {
            return error("新增供应商'" + obj.getSname() + "'失败，供应商名称已存在");
        }
		SysUser user = ShiroUtils.getSysUser();
    	obj.setCreateBy(user.getLoginName());
    	obj.setDept_id(user.getDeptId());
		return toAjax(service.insert(obj));
	}

	@GetMapping("/edit/{id}")
	public String edit(@PathVariable("id") Long id, ModelMap mmap) {
		Supplier obj = service.selectById(id);

		mmap.put("obj", obj);
		return prefix + "/edit";
	}

	/**
	 * 保存
	 */
	@Log(title = "供应商信息修改", businessType = BusinessType.UPDATE)
	@RequiresPermissions("dev:supplier:edit")
	@PostMapping("/edit")
	@ResponseBody
	public AjaxResult editSave(@Validated Supplier obj) {
		if (UserConstants.UNIQUE.equals(service.checkUnique(obj)))
        {
            return error("修改供应商'" + obj.getSname() + "'失败，供应商名称已存在");
        }
		obj.setUpdateBy(ShiroUtils.getLoginName());
		return toAjax(service.update(obj));
	}

	@RequiresPermissions("dev:supplier:delete")
	@Log(title = "供应商信息删除", businessType = BusinessType.DELETE)
	@PostMapping("/remove")
	@ResponseBody
	public AjaxResult remove(String ids) {
		try {
			return toAjax(service.deleteByIds(ids));
		} catch (Exception e) {
			return error(e.getMessage());
		}
	}
	
    @GetMapping("/selectSupplier/{sid}")
    public String selectSupplier(@PathVariable("sid") Long sid, ModelMap mmap)
    {
        mmap.put("obj", service.selectById(sid));
        return prefix + "/tree";
    }
    
    /**
     */
    @GetMapping("/treeData")
    @ResponseBody
    public List<Ztree> treeData()
    {
        List<Ztree> ztrees = service.selectSuppliertTree(new Supplier());
        return ztrees;
    }

    
}

...
@Controller
@RequestMapping("/op/dev/fix")
public class FixController extends BaseController {
	 @Autowired
	private IFixService service;
	
	private String prefix = "/dev/fix";
	
    @RequiresPermissions("dev:fix:view")
    @GetMapping()
    public String view()
    {
        return prefix + "/view";
     ...
     * 新增保存日报
     */
    @RequiresPermissions("dev:fix:add")
    ...
    @RequiresPermissions("dev:fix:edit")
    @PostMapping("/edit")
    @ResponseBody
    ...
    @RequiresPermissions("dev:fix:delete")

...
@Controller
@RequestMapping("/op/dev/fixOrder")
public class FixOrderController extends BaseController {
	 @Autowired
	private IFixOrderService service;
	
	private String prefix = "/dev/fixOrder";
	
    @RequiresPermissions("dev:fixOrder:view")
    @GetMapping()
    public String view()
    {
        return prefix + "/view";
    ...
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(FixOrder obj)
    {
        startPage();
        List<FixOrder> list = service.selectList(obj);
     ...
     * 新增保存日报
     */
    @RequiresPermissions("dev:fixOrder:add")
    @Log(title = "添加日报", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    @ResponseBody
    public AjaxResult addSave( FixOrder obj)
    {
    	SysUser user = ShiroUtils.getSysUser();
    	//SysDept sysDept = user.getDept();
    ...
    {
    	FixOrder obj = service.selectById(id);
    ...
    @RequiresPermissions("dev:fixOrder:edit")
    @PostMapping("/edit")
    @ResponseBody
    public AjaxResult editSave(@Validated FixOrder obj)
    {
    	obj.setUpdateBy(ShiroUtils.getLoginName());
        return toAjax(service.update(obj));
    }

    @RequiresPermissions("dev:fixOrder:delete")
